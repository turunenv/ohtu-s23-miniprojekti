...
class TestBibTexWriter(unittest.TestCase):
    ...
    def test_write_references_to_file_calls_file_io_write_the_correct_amount(self):
        self.bibtex_writer.write_references_to_file(
            "test.bib", self.references)
    ...
    def test_write_references_to_file_with_overwrite_only_overwrites_once(self):
        self.bibtex_writer.write_references_to_file(
            "test.bib", self.references, over_write=True)

        self.file_io.write.assert_any_call(
            "test.bib", self.references[0].create_bib_string(), True)
        self.file_io.write.assert_called_with(
            "test.bib", self.references[1].create_bib_string(), False)
    ...
    def test_write_references_to_file_returns_false_on_os_error(self):
        ...
        ret_val = self.bibtex_writer.write_references_to_file(
            "test.bib", self.references)
